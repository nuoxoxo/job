...
char    *get_words(cs& chunk)
{
        ...
        i = 0;
        if (!chunk[i])  return NULL ;	// wrong behavior if `(!chunk)` !!!
...
char    *new_chunk(cs &chunk)
{
        ...
        //words=(cs)malloc(sizeof(char)*(L(chunk) - i + 1)); /// CF. BELOW
        len_chunk = 0;
        while (chunk[len_chunk])    len_chunk ++;
        words = (cs) malloc(sizeof(char) * (len_chunk - i + 1));
...
char	*J(cs chunk, cs buff)
{
	...
	//if (!buff)  return NULL ;   /// ADDED LINE, NO APPARENT EFFECT
        if (!chunk)
        {
	...
	//size = L(chunk) + L(buff) + 1;    /// CHANGED TO BELOW
        
	int len_chunk, len_buff;
        len_chunk = len_buff = 0;
        if (chunk)
        {
            while (chunk[ len_chunk ])
                len_chunk ++;
        }
        if (buff)
        {
