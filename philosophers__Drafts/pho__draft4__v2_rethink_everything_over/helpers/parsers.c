...
{
	table->chops = malloc(sizeof(pthread_mutex_t) * ft_atoi(v[1])); // use atoi
	if (table->chops == NULL)
		exit (1);
		// clean_exit(NULL, table, 1); 
		// TODO : mem errors won't lead to freeing
	table->flag_of_death = 0; // in this ver. FoD not used in is_dead
	table->t0 = ft_current_ms();
	(void) c;
	// (void) v; // canceled by atoi, 1st line
}
